                ...
                <table class="min-w-[900px] w-full overflow-hidden">
                    <thead class="bg-slate-50 text-sm text-slate-700">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left whitespace-nowrap">
                                <button wire:click.prevent="sortBy('name')" class="flex items-center gap-1 hover:text-green-600">
                                    Tên phòng ban
                                    @if($sortField === 'name')
                                        <span class="text-xs">{{ $sortDirection === 'asc' ? '▲' : '▼' }}</span>
                                    @endif
                            ...
                            <th class="px-4 py-3 text-center whitespace-nowrap">Số nhân viên</th>
                    ...
                    <tbody class="text-sm sm:text-[15px]">
                        @forelse($departments as $department)
                            <tr class="transition hover:bg-slate-50">
                                <td class="border-t p-4 whitespace-nowrap">{{ $department->id }}</td>
                                ...
                                <td class="border-t p-4">
                                    <div class="max-w-[360px] truncate text-slate-600">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="border-t p-4 text-center text-gray-500">
                                    Không có dữ liệu
                                </td>
                            </tr>
                        @endforelse
