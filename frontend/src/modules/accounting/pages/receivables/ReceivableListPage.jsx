      ...
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Quản lý trực tiếp hóa đơn từ màn hình công nợ.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 px-6 py-5">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-700">Đơn bán hàng</label>
              <select
                name="sales_order_id"
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-700">Tổng tiền hóa đơn</label>
              <input
                name="total_amount"
                type="number"
                min="0"
...
function PaymentFormModal({ open, onClose, onSubmit, receivable, submitting }) {
  const getInitialForm = () => ({
    amount: receivable ? String(receivable.balance_amount || "") : "",
    payment_method: "",
    status: "paid",
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4">
      <div className="w-full max-w-xl rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Ghi nhận thu tiền</h3>
            <p className="text-sm text-slate-500">
              Hóa đơn: <span className="font-medium">{receivable.invoice_no}</span>
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 px-6 py-5">
          <div>
            <label className="mb-2 block text-sm font-medium text-slate-700">Số tiền thu</label>
            <input
              name="amount"
              type="number"
              min="0"
...
function ReceivableListPage() {
  ...
  const [payingReceivable, setPayingReceivable] = useState(null);
  const [invoiceModalSeed, setInvoiceModalSeed] = useState(0);
  const [paymentModalSeed, setPaymentModalSeed] = useState(0);
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message ||
          error?.message ||
          "Không thể tải dữ liệu tham chiếu công nợ"
      );
    }
  };
  ...
  const fetchReceivables = useCallback(async () => {
    ...
    try {
      const response = await receivableService.getList(params);
      setData(response?.data || {});
    } catch (error) {
      toast.error(
        error?.response?.data?.message || error?.message || "Không thể tải danh sách công nợ"
      );
    } finally {
      setLoading(false);
    }
  ...
  const openCreateInvoiceModal = () => {
    ...
    setInvoiceModalSeed((prev) => prev + 1);
  ...
  const openPaymentModal = (invoice) => {
    setPayingReceivable(invoice);
    setPaymentModalSeed((prev) => prev + 1);
  ...
  const handleSubmitPayment = async (form) => {
    ...
    try {
      await paymentService.create({
        invoice_id: Number(payingReceivable.id),
      ...
      toast.error(
        error?.response?.data?.message || error?.message || "Không thể ghi nhận thu tiền"
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Theo dõi công nợ phải thu và thao tác thu tiền trực tiếp trên từng hóa đơn.
              </p>
            </div>
          </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-2 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm text-slate-600">
            Tổng hóa đơn công nợ: <span className="font-semibold text-slate-900">{data?.total || 0}</span>
          </div>

          <div className="inline-flex items-center gap-2 rounded-xl bg-amber-50 px-3 py-2 text-sm font-medium text-amber-700">
            <FiDollarSign />
            Tổng dư nợ trang hiện tại: {formatCurrency(totalBalance)}đ
          </div>
        </div>

        {loading ? (
          <div className="px-5 py-10 text-center text-sm text-slate-500">Đang tải danh sách công nợ...</div>
        ) : receivables.length === 0 ? (
          <div className="px-5 py-10 text-center text-sm text-slate-500">Không có dữ liệu công nợ phù hợp.</div>
        ) : (
              ...
              <tbody>
                {receivables.map((item) => (
                  <tr key={item.id} className="border-t border-slate-100">
                    <td className="px-4 py-4 font-medium text-slate-700">{item.invoice_no}</td>
                    <td className="px-4 py-4 text-slate-600">{item.order_no || "-"}</td>
                    <td className="px-4 py-4 text-slate-700">{item.customer_name || "-"}</td>
                    <td className="px-4 py-4 text-right text-slate-700">
                      {formatCurrency(item.total_amount)}đ
                    </td>
                    <td className="px-4 py-4 text-right text-emerald-700">
                      {formatCurrency(item.paid_amount)}đ
                    </td>
                    <td className="px-4 py-4 text-right font-semibold text-rose-700">
                      {formatCurrency(item.balance_amount)}đ
                    </td>
                    ...
                    <td className="px-4 py-4 text-center">
                      <div className="flex flex-wrap items-center justify-center gap-2">
                        <button
                          ...
                          className="inline-flex items-center gap-2 rounded-lg border border-emerald-200 px-3 py-2 text-xs font-medium text-emerald-700 transition hover:bg-emerald-50"
                        >
                          <FiDollarSign />
                          Thu tiền
                        </button>
                        ...
                        <button
                          type="button"
                          onClick={() => openEditInvoiceModal(item)}
      ...
      <InvoiceFormModal
        key={`${editingInvoice?.id || "new"}-${invoiceModalSeed}`}
      ...
      <PaymentFormModal
        key={`${payingReceivable?.id || "new"}-${paymentModalSeed}`}
